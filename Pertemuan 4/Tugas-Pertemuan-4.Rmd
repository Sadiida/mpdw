---
title: "Pembangkitan Data AR, MA dan ARMA"
...
Akan dibangkitkan suatu data dengan model MA(2) sebanyak 300 data yang memiliki nilai $\theta_1 = 0.4$ dan $\theta_2 = 0.6$.
...
Berdasarkan pola segitiga nol pada plot EACF, terlihat bahwa segitiga nol berada pada ordo AR(0) dan ordo MA(2) meskipun tidak sempurna namun hal ini bisa mendukung model MA(2).
...
Berdasarkan scatterplot tersebut, terlihat bahwa terdapat hubungan negatif yang sangat lemah cenderung tidak berpola $Y_t$ dengan $Y_{t-1}$. Hal tersebut dibuktikan dengan nilai korelasi yang negatif dan nilai yang kecil:
...
Korelasi antara $Y_t$ dengan $Y_{t-3}$ dari hasil simulasi cukup mendekati nilai teoritis yang menyatakan bahwa $\rho_k=0$ untuk k\>q.

### Kesimpulan

Berdasarkan analisis, data bangkitan menunjukkan karakteristik yang sangat konsisten dengan model teoretis MA(2). Plot ACF cut off dengan jelas setelah lag 2, yang merupakan penanda utama model MA(2).

Konsistensi ini juga didukung oleh perbandingan nilai autokorelasi, di mana nilai empiris dari data bangkitan cukup mendekati nilai teoretisnya:
...
Meskipun tidak identik, perbedaan yang ada dapat diatribusikan pada variabilitas acak yang wajar terjadi pada simulasi data. Nilai ρ₃ yang kecil dan kemungkinan tidak signifikan secara statistik tetap sejalan dengan teori model MA(2) yang autokorelasinya menjadi nol setelah lag 2. Dengan demikian, dapat disimpulkan bahwa simulasi data telah berhasil merepresentasikan struktur dan pola korelasi dari sebuah proses MA(2) secara memadai.
...
Berdasarkan plot ACF tersebut terlihat bahwa plot ACF cenderung *tails off* sesuai dengan teori bahwa model AR akan tails off pada plot ACF.
...
Berdasarkan plot PACF tersebut, terlihat bahwa plot PACF *cuts off* pada lag pertama hal ini menandakan data mungin cocok untuk AR(1).
...
Korelasi antara $Y_t$ dengan $Y_{t-1}$ dari hasil simulasi sedikit mendekati perhitungan teoritis yaitu
...
Korelasi antara $Y_t$ dengan $Y_{t-2}$ dari hasil simulasi sedikit mendekati perhitungan teoritis yaitu:
...
Meskipun nilai autokorelasi empiris hingga lag ke-3 menunjukkan kedekatan dengan teori AR(2), pola PACF yang hanya menunjukkan signifikansi pada lag ke-1 mengindikasikan bahwa struktur korelasi data bangkitan lebih konsisten dengan model AR(1). Oleh karena itu, model AR(2) memiliki kecocokan kuantitatif, namun secara struktural model AR(1) mungkin lebih tepat untuk merepresentasikan dinamika data.
...
Berdasarkan pola segitiga nol pada plot EACF, terlihat bahwa segitiga nol berada pada ordo AR(4) dan ordo MA(0).
...
Berdasarkan scatterplot tersebut, terlihat bahwa tidak terlalu ada hubungan yang jelas antara $Y_t$ dengan $Y_{t-1}$.
...
Berdasarkan scatterplot tersebut, terlihat bahwa terdapat hubungan negatif lemah antara $Y_t$ dengan $Y_{t-2}$.
...
## Kesimpulan 
Selain itu, analisis dengan Extended Autocorrelation Function (EACF) menunjukkan bahwa segitiga nol pertama berada pada posisi AR(0) MA(4), yang justru mengindikasikan model kandidat yang lebih sederhana, yaitu MA(4). Hal ini diperkuat dengan pola ACF dan PACF dari data bangkitan yang sama-sama cut off pada lag ke-4, sehingga secara empiris lebih konsisten dengan model ARMA berorde rendah bukan ARMA(2,2). Dengan demikian, dapat disimpulkan bahwa meskipun model teoritis yang digunakan adalah ARMA(2,2), data bangkitan dan hasil analisis diagnostik (ACF, PACF dan EACF) menunjukkan kecocokan yang lebih baik dengan model yang lebih sederhana.